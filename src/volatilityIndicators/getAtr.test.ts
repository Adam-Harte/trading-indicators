...
describe('average true range', () => {
  it('should calculate the average true range based on the given parameters', () => {
    const atr = getAtr(highs, lows, 14);

    expect(atr).toBe(1.87);
  });

  it('should return null if the given period is greater than the high or low array', () => {
    const atr = getAtr(highs, lows, 30);

    expect(atr).toBe(null);
  });
});
