import { getUltOsc } from './getUltOsc';
...
describe('ultimate oscillator', () => {
  it('should calculate the ultimate oscillator based on the given parameters', () => {
    const ultOsc = getUltOsc(highs, lows, closes);

    expect(ultOsc).toBe(10.1);
  });

  it('should return null if the given period3 is greater than the high, close or low array', () => {
    const ultOsc = getUltOsc(highs, lows, closes, 7, 14, 35);

    expect(ultOsc).toBe(null);
  });
});
