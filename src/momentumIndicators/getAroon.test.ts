...
describe('aroon', () => {
  it('should calculate the aroon based on the given parameters', () => {
    const aroon = getAroon(highs, lows, 14);

    expect(aroon).toStrictEqual({ aroonDown: 35.71, aroonUp: 42.86 });
  });

  it('should return null if the given period is greater than the high or low array', () => {
    const aroon = getAroon(highs, lows, 32);

    expect(aroon).toBe(null);
  });
});
