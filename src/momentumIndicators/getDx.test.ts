...
describe('directional movement index', () => {
  it('should calculate the directional movement index based on the given parameters', () => {
    const dx = getDx(highs, lows, 14);

    expect(dx).toBe(53.8);
  });

  it('should return null if the given period + 1 is greater than the high or low array', () => {
    const dx = getDx(highs, lows, 30);
